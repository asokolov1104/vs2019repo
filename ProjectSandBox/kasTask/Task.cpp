/*
    ...
    Указать ошибки в данной программе: развернуто и подробно описать, в чем каждая из них заключается, как проявляется, к каким именно проблемам приводит. 
*/
...
namespace TSTTASK29
{
    ...
    void Check_errno(BOOL bResult, const char* file, int line)
    {
        ...
        if(!bResult)
        {
            std::wclog << L"An error " << errno << L" occured" << std::endl;
            std::wostringstream os;
            os  << L"OsError with errno code " << errno << L" occurred at " << (const wchar_t*)A2CW(file) << L"@" << line;
            throw OsError(os.str().c_str());
        };
    };

    //! check last error and throws an OsError exception if res is zero
    #define TSTTASK29_CHK_ERRNO(res)   TSTTASK29::Check_errno((res), __FILE__, __LINE__)
    ...
    #define TSTTASK29_CHK_GLE(res)   TSTTASK29::Check_GLE((res), __FILE__, __LINE__)
    ...
    */
    ...
    private:        
        ...
        static unsigned __stdcall thread_proc(void * p)
        {
            ...
            catch (const TSTTASK29::OsError& excpt)
            {
    ...
    //! Async action
    class DpcCall_0
        :   public DpcCallImplT<DpcCall_0>
    {
    public:
        DpcCall_0(ILogger* pOutput, IDpcCall* pControl)
            :   DpcCallImplT<DpcCall_0>(pControl)
        {
            m_pOutput = pOutput;
        };
...
int main()
{
    try
    {
        TSTTASK29::ApplicationImpl oApp;
    }
    catch (const TSTTASK29::OsError& excpt)
    {
        std::wcerr << excpt.what();
